...
public class SunriseSunsetAPI {
    ...
    private static class SunriseSunsetAPIQuery {
        ...
        public SunriseSunsetAPIQuery(LocalDate date, double latitude, double longitude, ZoneId timezone) {
            this.date = date;
    ...
    private static @NotNull SunriseSunsetData query(SunriseSunsetAPIQuery details) {
        ...
        try {
            ...
            uri = new URI(API_URL + formatter.format("?date=%s&lat=%f&lng=%f&timezone=%s&time_format=24",
                    details.date.format(DateTimeFormatter.ISO_LOCAL_DATE), details.latitude, details.longitude, details.timezone));
